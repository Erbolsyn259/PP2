...
class Apple:
    def __init__(self):
        self.size_apple = random.choice(range(40,121,40))
        self.apples1 = pygame.image.load('apple.png').convert_alpha()  # Load apple image with transparency
        self.apples = pygame.transform.scale(self.apples1, (self.size_apple, self.size_apple))  # Resize apple image
...
while True:
    ...
    if snake.head.x == apple.x and snake.head.y == apple.y:
        snake.body.append(pygame.Rect(squere.x, squere.y, block_size, block_size))  # Add a new segment to the snake
